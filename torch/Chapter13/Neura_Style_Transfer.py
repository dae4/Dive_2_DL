...
# %%
def gram(X):
    num_channels, n = X.shape[1], torch.tensor(X).size// X.shape[1]
    print(num_channels, n, X)
    X = torch.reshape(X, (num_channels, n))
...
def train(X, contents_Y, styles_Y, device, lr, num_epochs, lr_decay_epoch):
    ...
    return X
# %%
device, image_shape = try_gpu(), (150, 225) # PIL Image (h, w)
net = net.to(device)
...
plt.imsave('../img/neural-style.jpg', postprocess(output))
# %%
